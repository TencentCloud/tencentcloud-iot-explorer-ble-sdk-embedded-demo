...
static int ble_property_power_switch_set(const char *data, uint16_t len)
{
	uint8_t tmp_bool = 0;
	tmp_bool = data[0];
	printf("set id power_switch bool value %02x\n", data[0]);
	return 0;
}
...
static int ble_property_color_set(const char *data, uint16_t len)
{
	...
	printf("set id color int value %d\n", tmp_enum);
	return 0;
}
...
ble_property_t sg_ble_property_array[4] = {
...
static int ble_action_handle_light_blink_input_cb(e_ble_tlv *input_param_array, uint8_t input_array_size,uint8_t *output_id_array)
{
	int i = 0;
...
static int ble_action_handle_light_blink_output_cb(uint8_t output_id, char *buf, uint16_t buf_len)
{
	//get value of the output id, return actual length
	return 0;
}

static uint8_t sg_ble_action_light_blink_input_type_array[3] = {
	BLE_QIOT_DATA_TYPE_INT,
	BLE_QIOT_DATA_TYPE_BOOL,
	BLE_QIOT_DATA_TYPE_INT,
};

static uint8_t sg_ble_action_light_blink_output_type_array[1] = {
	BLE_QIOT_DATA_TYPE_ENUM,
};

ble_action_t sg_ble_action_array[1] = {
	{ble_action_handle_light_blink_input_cb, ble_action_handle_light_blink_output_cb,sg_ble_action_light_blink_input_type_array, sg_ble_action_light_blink_output_type_array,3,1},
};
